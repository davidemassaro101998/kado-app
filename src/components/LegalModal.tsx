...
export const LegalModal: React.FC<LegalModalProps> = React.memo(({
  ...
  const getIcon = () => {
    if (type === "privacy") return <ShieldCheck className="w-5 h-5 text-[#FF3D7F]" />;
    if (type === "terms") return <FileText className="w-5 h-5 text-[#FF3D7F]" />;
    return <ShoppingBag className="w-5 h-5 text-[#FF3D7F]" />;
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
        <motion.div
          ...
          className="bg-[#17111A] text-[#F7F0F2] rounded-[28px] p-5 sm:p-6 max-w-lg w-full border border-[#2B2130] shadow-2xl flex flex-col max-h-[85vh] overflow-hidden gpu-layer"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-[#2B2130] pb-3 mb-3 shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="font-extrabold text-base text-[#F7F0F2]">
                {getTitle()}
              </h3>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto space-y-3.5 text-xs leading-relaxed font-normal text-[#F7F0F2] pr-1">
            {type === "privacy" && (
              <>
                <p className="font-bold text-sm text-[#FF3D7F]">
            ...
            {type === "affiliate" && (
              <>
                ...
                <div className="p-3.5 rounded-2xl bg-[#1C1520] border border-[#2B2130] font-semibold text-xs text-[#F7F0F2] space-y-2">
                  <p>
                    "In qualità di Affiliato Amazon, Kado AI riceve un guadagno dagli acquisti idonei."
                  </p>
                  <p className="text-[11px] font-normal text-[#9B8A93]">
                ...
                <div className="p-3.5 rounded-2xl bg-[#1C1520] border border-[#2B2130] space-y-1.5">
                  <span className="font-bold text-xs text-[#F7F0F2] block">
                    Disclaimer Prezzi e Disponibilità:
                  </span>
                  <p className="text-[11px] text-[#9B8A93] leading-relaxed">
          ...
          <div className="pt-3 border-t border-[#2B2130] mt-3 flex justify-end shrink-0">
            <button
              onClick={onClose}
              className="py-2.5 px-5 rounded-xl bg-[#FF3D7F] text-white font-bold text-xs cursor-pointer hover:bg-[#E02968] transition-colors"
            >
              {isIt ? "Chiudi" : "Close"}
            </button>
          </div>
